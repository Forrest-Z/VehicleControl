...
Status LQRController::Init(ControlConf &control_conf)
{
    name = "LQR controller";
...
void LQRController::ComputeStateError(prius_msgs::My_Trajectory_Point &goal_state,VehicleState &vehicle_state)
{
...
double LQRController::GetPreviewSteeringAngle(prius_msgs::My_Trajectory_Point point)
{
...
void LQRController::LQRSolverWithPrediction(TrajectoryAnalyzer &trajectory_analyzer,VehicleState &vehicle_state)
{
    int num=1;
    ...
    for(int i= LQR_preview_trajectory.size()-1;i>=0;i=i-iteration_interval)
    {
        ...
        UpdateKineticModel(current_velocity,LQR_preview_trajectory[i].theta,steering_angle_in_trajectory);
        Pk_new = Pk;
        Vk_new = Vk;
...
void LQRController::UpdateKineticModel(double current_velocity,double heading,double steering_angle)
{
    A(0,2) = -current_velocity*sin(heading)*Ts;
    A(1,2) = current_velocity*cos(heading)*Ts;
    B(2) = current_velocity*Ts/(wheel_base*cos(steering_angle)*cos(steering_angle));
}
...
void LQRController::LQRSOlver(TrajectoryAnalyzer &trajectory_analyzer,VehicleState &vehicle_state)
{
    double goal_steering_angle = GetPreviewSteeringAngle(trajectory_analyzer.goal_state);
    UpdateKineticModel(current_velocity,trajectory_analyzer.goal_state.theta,goal_steering_angle);
    SolveLQRProblem(solve_tolerance, max_iteration_num);
}

Status LQRController::ComputeControlCmd(TrajectoryAnalyzer &trajectory_analyzer,VehicleState &vehicle_state,prius_msgs::Control &control_cmd)
{
    ...
    {
        LQRSOlver(trajectory_analyzer,vehicle_state);
